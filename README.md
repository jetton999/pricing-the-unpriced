...
Nine CSV tables, a `START_HERE` walkthrough, `DATA_DICTIONARY.md`, the full proposal PDF,
and `verify_claims.py`. The `map/` folder is a standalone map of every property with its
history, people, and registered IP; run `python3 map/app.py` and open the printed URL.
`docs/` holds background research for the ideas. `public_sources/` holds the 1937 HOLC
redlining polygons for Baltimore. `site/` is the revised proposal as a single web page;
open `site/index.html`.

## Running things

The map app and the claims checker use only the Python standard library. No install.

The walkthrough needs pandas, matplotlib, and networkx. We recommend [uv](https://docs.astral.sh/uv/),
which fetches them into a throwaway environment with no setup:

```bash
uvx --with pandas --with matplotlib --with networkx jupyter lab START_HERE.ipynb
```

`START_HERE.py` and `START_HERE.ipynb` are the same notebook, paired with jupytext. The
`.py` is the source of truth: edit it, or have a coding agent edit it, then sync the
notebook view with `uvx jupytext --sync START_HERE.py`. Coding agents handle plain Python
far more reliably than notebook JSON; `docs/students-and-coding-agents.md` has the details.

Without uv, `pip install pandas matplotlib networkx jupyter` and `jupyter lab` work the
same way.

The proposal PDF is the sponsor's original CUSP submission. It framed the capstone around
scoring each property's documented history and testing that score against sale prices.
The two ideas above are the sponsor's current direction and take precedence where they
differ.

Read the incident table carefully: administrative feeds and curated history are mixed in
one file, and separating them is the first job. Assessed value is an administrative
number, not a market price.

Sponsor data is CC BY 4.0. Third-party files in `/public_sources` carry their own terms.
See `LICENSE.md`.
