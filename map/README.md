...
per-source layer table, and exits 1 if properties ≠ 673, incidents ≠ 10,614, or curated ≠ 3,597.

Markers are the 592 properties with coordinates, coloured by curated depth (0 / 1–9 / 10–24 / 25+).
...
*curated*. That gives 3,597 / 7,017. Two sources cut against the "namespaced = administrative"
heuristic: `hud:cdbg` (36 rows) is namespaced but counted as curated, and `sdat_assessments`
(850) is a bare key but is an assessment roll, so administrative. Moving `hud:cdbg` would give
